...
void Credits::Draw() {
    ...
    //
    if (fullscreen) {
        fullscreen->brightness = 1.0; //glm::smoothstep(0.0f, 1.0f, glm::min(1.0f, time / screenFadeTime));
    }
    ...
    if (not fixedCam) {
        ...
        cam.center = cameraPoints.After(time);
    } else {
        float t = time; 
    ...
    glm::mat4 loc;// = glm::rotate(glm::mat4(), glm::vec3(0, -1.5, 0));
    cam.Bind(loc);
    glUniform3f(u("Eye"), cam.eye.x, cam.eye.y, cam.eye.z);

    glUniform3f(u("MaterialColor"), .0, .0, .0);
    glUniform1f(u("Alpha"), .5);
    glUniform1f(u("Time"), time);
    //_tube.Draw();

    static float m = .1f;
    if (GetContext()->audio->GetKicks() or GetContext()->audio->GetSnares()) {
        m = .5;
    }
    ...
    for (int i = 0; i < 4; i ++) {
        loc = glm::translate(glm::mat4(), m*glm::vec3(sin(7*(i+1)+200*time), sin(7*(i+1)+200*time), 0));
        cam.Bind(loc);
        FOR_EACH(tubeIt, _tubes) {
            //if (tubeIt->cvs.size() == _tube.cvs.size())
            //    continue;
            tubeIt->Draw();
        }
    }
